 ...
 * when created using the DefaultCacheFactory</li> <li>When {@link org.jboss.cache.Cache#create()} is called, the
 * components are rewired.</li> <li>STARTED - when {@link org.jboss.cache.Cache#start()} is called</li> <li>STOPPED -
 * when {@link org.jboss.cache.Cache#stop()} is called</li> <li>DESTROYED - when {@link org.jboss.cache.Cache#destroy()}
 * is called.</li> </ul>
 * <p/>
...
public class ComponentRegistry implements Lifecycle {
   ...
   /**
    * Wires an object instance with dependencies annotated with the {@link org.jboss.cache.factories.annotations.Inject}
    * annotation, creating more components as needed based on the Configuration passed in if these additional components
    * don't exist in the {@link ComponentRegistry}.  Strictly for components that don't otherwise live in the registry
   ...
   /**
    * This is hard coded for now, since scanning the classpath for factories annotated with {@link
    * org.jboss.cache.factories.annotations.DefaultFactoryFor} does not work with all class loaders.  This is a
    ...
    * may be injected from an external system. 3.  Look for a class definition passed in to the {@link
    * org.jboss.cache.config.Configuration} - such as an EvictionPolicy implementation 4.  Attempt to create it by
   ...
   /**
    * Scans the class path for classes annotated with {@link org.jboss.cache.factories.annotations.DefaultFactoryFor},
   ...
   /**
    * Stops the cache and sets the cache status to {@link org.jboss.cache.CacheStatus#STOPPED} once it is done.  If the
    * cache is not in the {@link org.jboss.cache.CacheStatus#STARTED} state, this is a no-op.
    */
   public void stop() {
    ...
    * done.
    * <p/>
    * If the cache is in {@link org.jboss.cache.CacheStatus#STARTED} when this method is called, it will first call
    ...
    * org.jboss.cache.config.Configuration#getStateRetrievalTimeout()} millis, checking for a valid state.
    *
    * @param originLocal true if the call originates locally (i.e., from the {@link org.jboss.cache.invocation.CacheInvocationDelegate}
    *                    or false if it originates remotely, i.e., from the {@link org.jboss.cache.marshall.CommandAwareRpcDispatcher}.
   ...
   /**
    * Blocks until the current cache instance is in its {@link org.jboss.cache.CacheStatus#STARTED started} phase.
    * Blocks for up to {@link org.jboss.cache.config.Configuration#getStateRetrievalTimeout()} milliseconds, throwing an
