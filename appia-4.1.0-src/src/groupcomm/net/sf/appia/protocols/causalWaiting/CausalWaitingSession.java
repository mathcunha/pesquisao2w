...
public class CausalWaitingSession extends Session {
    ...
    private static Logger log = Logger.getLogger(CausalWaitingSession.class.getName());
	...
	private void handleView(View view) {
		...
		// Sanity check
		if (!pending.isEmpty()) {
			log.log(Level.SEVERE, "Received new view but pending messages still exist! View synchrony properties compromised!");
	...
	private void handleGroupSendableEvent(GroupSendableEvent event) {
        if(debugOn && Level.FINEST.equals(log.getLevel()))
            log.log(Level.FINEST, "CAUSAL Processing event "+event);
