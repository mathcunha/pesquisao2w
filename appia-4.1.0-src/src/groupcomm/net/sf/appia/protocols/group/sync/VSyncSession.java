...
public class VSyncSession extends Session {
    private static Logger log = Logger.getLogger(VSyncSession.class.getName());
    ...
    private void handleBlock(Block ev) {
        ...
        if (syncCoord == ls.my_rank) {
            ...
            if (type != BLOCKED) {
                log.log(Level.FINEST, "Invalid Block type (received="+type+", expected="+BLOCKED+") from "+ev.orig+" to coordinator");
                return;
            }
            ...
            if (type != BLOCK) {
                log.log(Level.FINEST, "Invalid Block type (received="+type+", expected="+BLOCK+") from "+ev.orig+" to other");
                return;
            }
    ...
    private void handleSync(Sync ev) {
        ...
        if (syncCoord == ls.my_rank) {
            ...
            if (type != SYNCHED) {
                log.log(Level.FINEST, "Invalid Sync type (received="+type+", expected="+SYNCHED+") from "+ev.orig+" to coordinator");
                return;
            }
            ...
            if (type != SYNC) {
                log.log(Level.FINEST, "Invalid Sync type (received="+type+", expected="+SYNC+") from "+ev.orig+" to other");
                return;
            }
            ...
            if (debugFull) {
                log.log(Level.FINEST, "{"+ls.my_rank+"} Received Sync from "+ev.orig);
    ...
    private void handleGroupSendableEvent(GroupSendableEvent ev) {
        ...
        if (ev instanceof Send) {
            if (ev.getDir() == Direction.DOWN) {
                ...
                if ((global_sends != null) && (my_sends_recv[ev.orig] == global_sends[ev.orig])) {
                    log.log(Level.FINEST, "Send Event from "+ev.orig+" (failed="+ls.failed[ev.orig]+") discarded because it was greater than global");
                    return;
                }
        ...
        if (ev.getDir() == Direction.DOWN) {
            ...
            if ((global_casts != null) && (my_casts[ev.orig] == global_casts[ev.orig])) {
                log.log(Level.FINEST, "Cast Event from "+ev.orig+" (failed="+ls.failed[ev.orig]+") discarded because it was greater than global");
            }
            else{
                my_casts[ev.orig]++;
                try { ev.go(); } catch (AppiaEventException ex) { ex.printStackTrace(); }
    ...
    private void coord_handleSync(int orig, long[] casts, Channel channel) {
        if (sync == null) {
            log.log(Level.FINEST, "Received Sync out of time. Ignored.");
            return;
        }
    ...
    private void in_block(Channel channel) {
        ...
        if (debugFull) {
            log.log(Level.FINEST, "{"+ls.my_rank+"} Sending my_casts and my_sends_sent to coordinator");
            debugLongArray("my_casts=",my_casts);
    ...
    private void in_sync(Channel channel) {
        if (debugFull) {
            log.log(Level.FINEST, "testing in_sync");
            log.log(Level.FINEST, "in_sync: my_casts==global_casts -> "+equals(my_casts,global_casts,null));
            log.log(Level.FINEST, "in_sync: my_sends_recv== global_sends -> "+equals(my_sends_recv,global_sends,ls.failed));
        }
    ...
    private void begin_blocking(Channel channel) {
        ...
        if (debugFull)
            log.log(Level.FINEST, "Started blocking");
    ...
    private void end_blocking(Channel channel) {
        ...
        if (debugFull) {
            log.log(Level.FINEST, "{"+ls.my_rank+"} Ended blocking started synching");
            debugLongArray("global_casts=",global_casts);
