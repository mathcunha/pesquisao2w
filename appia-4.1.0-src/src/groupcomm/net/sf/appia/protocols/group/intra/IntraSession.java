...
public class IntraSession extends Session {
    private static Logger log = Logger.getLogger(IntraSession.class.getName());
  ...
  private void handleInstallView(InstallView ev) {
    ...
    if (ls.failed[ev.orig]) {
      log.log(Level.FINEST, "discarded InstallView because it came from a failed member");
      return;
    }
    ...
    if (new_vs.getRank(my_endpt) < 0) {
      log.log(Level.FINEST, "discarded InstallView because i don't belong to view");
      return;
    }
    ...
    if (new_ls.my_rank < k) {
      try {
        ...
        ex.printStackTrace();
        log.log(Level.WARNING, "impossible to resend InstallView");
      }
    }
  ...
  private void handleFail(Fail ev) {
    ...
    if (ls.am_coord && !new_view) {
      if (debugFull)
        log.log(Level.FINEST, "Started view change due to Fail");
  ...
  private void handleViewChange(ViewChange ev) {
    if (ls.am_coord && !new_view) {
      if (debugFull)
        log.log(Level.FINEST, "Started view change due to ViewChange");
  ...
  private void sendInstallView(Channel channel, ViewState new_vs, int k) {
    try {
      ...
      ex.printStackTrace();
      log.log(Level.WARNING, "impossible to send InstallView");
    }
  }
