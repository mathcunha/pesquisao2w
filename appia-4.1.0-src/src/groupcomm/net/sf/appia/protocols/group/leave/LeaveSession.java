...
public class LeaveSession extends Session {
    private static Logger log = Logger.getLogger(LeaveSession.class.getName());
  ...
  private void handleExitEvent(ExitEvent ev) {
    if ((ev.getMessage().popInt() != vs.group.hashCode()) ||
      ...
      log.log(Level.FINEST, "Exit discarded due to bad Group and/or ViewID");
      return;
    }
    
    if (!to_leave[ls.my_rank]) {
      log.log(Level.FINEST, "Exit discarded because i didn't request to leave.");
      return;
    }
