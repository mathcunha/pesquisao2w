...
public class PhiSuspectSession extends AbstractSensorSession implements InitializableSession {
    ...
    private static Logger log = Logger.getLogger(PhiSuspectSession.class.getName());
...
//    }
    ...
    private void handleAliveEvent(Alive ev) {
        ...
        if (ev.getDir() == Direction.UP) {
            if (debugFull)
                log.log(Level.FINEST, "Recv msg from "+ev.orig+"@"+time.currentTimeMillis());
    ...
    private void handleSuspect(Suspect ev) {
        ...
        if (ev.failed[ls.my_rank]) {
            log.log(Level.FINEST, "i am not failed, but someone suspected me!!");
            return;
        }
...
//				double elapsed = now - e.getValue().lastTimeReceived_;
				...
				log.log(Level.FINEST, "Suspected "+e.getKey()+" because its phi is " + phi);
			}
		}
