...
public class RemoteGossipOutSession extends GossipOutSession {
    ...
    private static Logger log = Logger.getLogger(RemoteGossipOutSession.class.getName());
	...
	private void handleRemoteView(RemoteViewEvent ev) {
		...
		try {
			...
			if(Level.FINEST.equals(log.getLevel()))
				log.log(Level.FINEST, "Received remote view event from "+sourceAddr+" with group "+g+" ("+group+")");
			if (!group.equals(g)) {
				return;
			}
			...
			if(Level.FINEST.equals(log.getLevel()))
				log.log(Level.FINEST, "Sendig RemoteView to "+sourceAddr+" : "+getViewState());
			ev.setMessage(msg);
            ...
            log.log(Level.FINEST, "Exception sending event: "+ex);
		}
	}
}
