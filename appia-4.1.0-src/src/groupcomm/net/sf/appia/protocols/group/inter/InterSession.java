...
public class InterSession extends Session implements InitializableSession {
    private static Logger log = Logger.getLogger(InterSession.class.getName());
    ...
    private void handleConcurrent(ConcurrentViewEvent ev) {
        ...
        if (sent_preview) {
            log.log(Level.FINEST, "Concurrent view warning discarded because i already sent PreView"+
            ". Wait for next view.");
            return;
        }

        if ((myInfo != null) && myInfo.decided) {
            log.log(Level.FINEST, "Concurrent view warning discarded because i have already decided.");
            return;
        }

        if (find(ev.id) >= 0) {
            log.log(Level.FINEST, "Received duplicate concurrent view warning. Ignoring it.");
            return;
        }
        ...
        if (!validateProposal()) {
            log.log(Level.FINEST, "Generated invalid proposal. Aborting merge.");
    ...
    private void handleMergeTimer(MergeTimer ev) {
        if (ev.timerID.equals("WAIT "+timerID+" "+this)) {
            log.log(Level.FINEST, "Wait timer expired.");
            waited=true;
            decide(ev.getChannel());
        } else if (ev.timerID.equals("TERMINATE "+timerID+" "+this)) {
            log.log(Level.FINEST, "Terminate timer expired.");
            if (views.size() > 0) {
                sendAbort(ev.getChannel());
                reset();
            }
    ...
    private void handleMergeEvent(MergeEvent ev) {
        ...
        case MergeEvent.PROPOSE:
            ...
            if (same && (r == round)) {
                log.log(Level.FINEST, "Received identical proposal from "+vids[sender]+"("+addrs[sender]+") with address "+ev.source);
                receiveIdenticalProposal(sender, ev.getChannel());
            } else {
                log.log(Level.FINEST, "Received new proposal from "+vids[sender]+"("+addrs[sender]+") with address "+ev.source);
                if (debugFull)
        ...
        case MergeEvent.DECIDE:
            ...
            if (same && (r == round)) {
                log.log(Level.FINEST, "Received identical decide from "+vids[sender]+"("+addrs[sender]+") with address "+ev.source);
                receiveIdenticalDecide(sender,sender_vs);
            } else {
                log.log(Level.FINEST, "Received new decide from "+vids[sender]+"("+addrs[sender]+") with address "+ev.source);
                if (debugFull)
        ...
        case MergeEvent.ABORT:
            ...
            if (find(sender_id) < 0) {
                log.log(Level.FINEST, "Received abort from "+sender_id+" with address "+ev.source);
                sendAbort(ev.getChannel());
                reset();
            } else {
                log.log(Level.FINEST, "Received abort from unknown sender ("+sender_id+"). Ignoring it.");
            }
            break;
        }
    }
    ...
    private boolean receiveNewProposal(int sender, int r, ViewID[] vids, InetSocketAddress[] addrs, Channel channel) {
        if (sent_preview) {
            log.log(Level.FINEST, "Received new proposal but already sent PreView");
        ...
        if (views.size() == 0) {
            ...
            if ((myIndex=find(vs.id)) < 0) {
                log.log(Level.FINEST, "Received new proposal but i don't belong. Ignoring it.");
                reset();
                return false;
            } 
            ...
            if ((newElements > 0) || (round > r)) {
                ...
                if (commonElements == originalSize) {
                    round=r;
                    log.log(Level.FINEST, "Received new updated proposal.");
                } else {
                    round=(r > round ? r : round)+1;
                    myInfo.proposed=false;
                    log.log(Level.FINEST, "Created new proposal round.");
                } 
            } else {
                log.log(Level.FINEST, "Received old proposal. Ignoring it.");
                return false;
            }
        }
        ...
        for (i=0 ; i < views.size() ; i++) {
            ...
            for (k=0 ; k < vs.previous.length ; k++) {
                if (info.id.equals(vs.previous[k])) {
                    if (debugFull)
                        log.log(Level.FINEST, "validateProposal: has previous.");
                    return false;
                }
            }
            for (j=i+1 ; j < views.size() ; j++) {
                ...
                if (info.addr.equals(aux.addr)) {
                    if (debugFull)
                        log.log(Level.FINEST, "validateProposal: duplicate address.");
                    return false;
                }
            }
        }
    ...
    private void sendViewChange(Channel channel) {
        try {
            ...
            ex.printStackTrace();
            log.log(Level.FINEST, "Impossible to send \"ViewChange\"");
        }
    }
  ...
  public static void main(String[] args) {
    ...
    private void debugViews(String s) {
        if (Level.FINEST.equals(log.getLevel())) {
            ...
            log.log(Level.FINEST, "appia:group:InterSession:VIEWS("+round+"): "+s);
            while (iter.hasNext()) {
                ViewInfo info=(ViewInfo)iter.next();
                log.log(Level.FINEST, "\t-> "+info.id);
                log.log(Level.FINEST, "\t\tproposed = "+info.proposed);
                log.log(Level.FINEST, "\t\tdecided = "+info.decided);
                log.log(Level.FINEST, "\t\tvs = "+(info.vs != null ? "filled" : "null"));
                log.log(Level.FINEST, "\t\taddress = "+info.addr);
            }
        }
    }
    ...
    private void debugProposal(String s, int r, int n, ViewID[] vids, InetSocketAddress[] addrs) {
        if (Level.FINEST.equals(log.getLevel())) {
            ...
            for (int x=0 ; x < addrs.length ; x++)
                s+=addrs[x]+",";
            s+="}";
            log.log(Level.FINEST, "appia:group:InterSession: "+s);
        }
    }
}
