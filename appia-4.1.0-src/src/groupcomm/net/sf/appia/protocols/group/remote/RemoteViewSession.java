...
public class RemoteViewSession extends Session implements InitializableSession {
    ...
    private static Logger log = Logger.getLogger(RemoteViewSession.class.getName());
	...
	public void init(SessionProperties params) {
	    if (params.containsKey("gossip")) {
	        try {
	            ...
	            log.log(Level.WARNING, "XML initialization failed due to the following exception: "+e);
	...
	public void handle(Event event) {
		...
		if (event instanceof Debug) {
			...
			if (ev.getQualifierMode() == EventQualifier.ON) {
				...
				log.log(Level.FINEST, "Full debugging started.");
			} else {
			...
			try { 
                ...
                log.log(Level.FINEST, "error forwarding event of type "+ev.getClass().getName()+" : "+ex);
			}
			return;
		} // end of debug event handling
		
		log.log(Level.WARNING, "Received unwanted event (" + event.getClass().getName()+"). Forwarding it.");
		try {
		    event.go(); 
		} catch (AppiaEventException ex) {
            log.log(Level.FINEST, "error forwarding event of type "+event.getClass().getName()+" : "+ex);
		}
	}
		...
		if (myChannel == null) {
			...
			if (needsRse) {
				try {
                    ...
                    log.log(Level.FINEST, "error forwarding event of type "+RegisterSocketEvent.class.getClass().getName()+" : "+ex);
				}
			}	    
			...
			try {
                ...
                log.log(Level.FINEST, "error forwarding event of type "+GossipOutEvent.class.getName()+" : "+ex);
			}
		}
	}
	...
	private void makeOutChannel(Channel t) {
		...
		try {
			...
			log.log(Level.FINEST, "Error: unable to create GossipOut channel: "+ex);
	...
	private void handleGossipOut(GossipOutEvent event) {
		...
		try{
            ...
            log.log(Level.FINEST, "error forwarding event of type "+event.getClass().getName()+" : "+e);
		}
	}
		...
		if (event.getDir() == Direction.DOWN) {
			...
			try {
				if(Level.FINEST.equals(log.getLevel()))
					log.log(Level.FINEST, "sending request from " + myAddress + 
...
//				event.setMessage(msg);
				...
				log.log(Level.FINEST, "error sending down RemoteViewEvent: "+ex);
			}
		} else {
			...
			try{
			    ...
			    if(Level.FINEST.equals(log.getLevel()))
			        log.log(Level.FINEST, "Message contents do not appear to be a ViewState object.");
	...
	private void handleRegisterSocketEvent(RegisterSocketEvent e){
		...
		try{
            ...
            log.log(Level.FINEST, "error forwarding event of type "+e.getClass().getName()+" : "+e);
		}
	}
