...
public class StableStorage {
    private static Logger log = Logger.getLogger(StableStorage.class.getName());
    ...
    public void clean(int rank, long seqno) {
        ...
        while ((h.first != null) && (h.first.seqno <= seqno)) {
            ...
            if (debugFull) {
                log.log(Level.FINEST, "Cleaned "+h.first.seqno+" from "+rank);
            }
  ...
  private void debug() {
    ...
    for (i=0 ; i < storage.length ; i++) {
      ...
      log.log(Level.FINEST, "\t DEBUG STORAGE rank "+i);

      for (aux=storage[i].first ; aux != null ; aux=aux.next) {
        log.log(Level.FINEST, "\t\t DEBUG STORAGE seqno "+aux.seqno);
      }
    }
  }
     */
