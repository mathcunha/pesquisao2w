...
public class GossipOutSession extends Session implements InitializableSession {
    private static Logger log = Logger.getLogger(GossipOutSession.class.getName());
    ...
    private void handleGossipOutEvent(GossipOutEvent ev) {
        ...
        if ((ev.getChannel() != out) && (ev.getDir() == Direction.UP)) {
            log.log(Level.FINEST, "Received upward GossipOutEvent. That's very strange but forwarding it !!!!");
            try { ev.go(); } catch (AppiaEventException ex) { ex.printStackTrace(); }
            return;
        }
        ...
        if ((out != null) && (ev.getChannel() != out)) {
            if (ev.dest != null)
                log.log(Level.FINEST, "GossipOut has destination address. Ignoring it and sending to server.");

            if (server < 0) {
                log.log(Level.WARNING, "No server available, discarding message");
                return;
            }
            ev.dest=gossipAddrs[server];

            if (debugFull)
                log.log(Level.FINEST, "placing my address "+outAddr.toString()+" on message to "+ev.dest);
            ...
            try {
                ...
                ex.printStackTrace();
                log.log(Level.WARNING, "Impossible to send gossip in \"out\" channel");
            }
        ...
        if ((ev.getChannel() == out) && (in != null)) {
            ...
            if (debugFull)
                log.log(Level.FINEST, "Received message from "+ev.source+" for channel \""+channelName+"\".");

            Channel channel=(Channel)in.get(channelName);
            if (channel == null) {
                log.log(Level.FINEST, "Received message for unknown channel ("+channelName+") ignoring it.");
                return;
            }
            ...
            try {
                ...
                if (debugFull)
                    ex.printStackTrace();
                log.log(Level.WARNING, "Impossible to send gossip in \"in\" channel");
            }
    ...
    private void createOutChannel(Channel channel) {
        try {
            ...
            if (debugFull) {
                log.log(Level.FINEST, "Sharing UDP ? "+(udp_layer_class == null)+" (share_udp="+share_udp+" udp_layer="+udp_layer_name+" main_bottom_layer="+main_bottom_layer+")");
            }


        } catch (AppiaCursorException ex) {
            ex.printStackTrace();
            log.log(Level.WARNING, "Unable to create \"out\" channel, GossipOutEvents will be lost");
    ...
    private void register() {
        ...
        if (requiresRSE) {
            try {
                ...
                ex.printStackTrace();
                log.log(Level.WARNING, "Unable to register Gossip Out socket. Unable to locate concurrent views. Unable to merge.");
            }
        }
        ...
        try {
            ...
            ex.printStackTrace();
            log.log(Level.WARNING, "Unable to config Fifo layer. It will work but not optimized");
        }
            ...
            for (int i=0 ; i < gossipAddrs.length ; i++) {
                if (gossipAddrs[i].getAddress().isMulticastAddress()) {
                    try {
                        ...
                        log.log(Level.FINEST, "Registering gossip multicast address: "+gossipAddrs[i]);
                    } catch (AppiaEventException ex) {
                        ex.printStackTrace();
                        log.log(Level.WARNING, "Impossible to register multicast address.");
                    }
                }
            }
        }
