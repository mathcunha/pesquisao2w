...
public class SslCompleteSession extends TcpCompleteSession implements InitializableSession {
  ...
  private static Logger log = Logger.getLogger(SslCompleteSession.class.getName());
  ...
  public void init(SessionProperties props){
      ...
      if(Level.FINEST.equals(log.getLevel()))
          log.log(Level.FINEST, "SSL parameters after XML init:\n"+
  ...
  private void handleSendable(SendableEvent e){
    ...
    if(Level.FINEST.equals(log.getLevel()))
        log.log(Level.FINEST, "Preparing to send event "+e);
  ...
  private boolean validate(InetSocketAddress dest, Channel channel) {
    ...
    try {
      ...
      if(existsSocket(ourReaders,dest)){
        if(Level.FINEST.equals(log.getLevel()))
            log.log(Level.FINEST, "recognized our ssl socket. sending...");
        return true;
      }
      else{//if not
        ...
        if(existsSocket(otherReaders,dest)){
          if(Level.FINEST.equals(log.getLevel()))
              log.log(Level.FINEST, "recognized other ssl socket. sending...");
          return true;
        }
        else{//if not
          ...
          if(createSSLSocket(ourReaders,dest,channel) != null)
            if(Level.FINEST.equals(log.getLevel()))
                log.log(Level.FINEST, "created new ssl socket, sending...");
          return true;
        }
      }
      ...
      if(Level.FINEST.equals(log.getLevel())) {
        ex.printStackTrace();
        log.log(Level.FINEST, "Member "+dest.toString()+" has failed.");
      }
...
//      removeSocket(dest);
  ...
  private void handleSslRegisterSocket(SslRegisterSocketEvent e){
    if(Level.FINEST.equals(log.getLevel()))
        log.log(Level.FINEST, "Received SSL register socket event: "+e);
  ...
  private int registerWithSSL(int port, Channel channel){
      ...
      try{
        ...
        if(Level.FINEST.equals(log.getLevel()))
            ex.printStackTrace();
        log.log(Level.WARNING, "An error ocurred when initializing SSL session though the SSL register socket event: "+ex.getMessage());
        return -1;
      }
      ...
      if (port > 0) {
        ...
        if(Level.FINEST.equals(log.getLevel()))
          log.log(Level.FINEST, "Local port is "+ourPort);
      }
      else
          ourPort = -1;
      return ourPort;
  ...
  protected Socket createSSLSocket(Hashtable<InetSocketAddress,SocketInfoContainer> hm,
    ...
    synchronized(socketLock){
      ...
      if(Level.FINEST.equals(log.getLevel()))
        log.log(Level.FINEST, "Sending our original port "+ourPort);
