...
public class UdpSimpleSession extends Session implements InitializableSession {
    private static Logger log = Logger.getLogger(UdpSimpleSession.class.getName());
    private static Logger logReader = Logger.getLogger(UdpSimpleReader.class.getName());
...
//  private ThreadFactory threadFactory = null;
  ...
  public UdpSimpleSession(Layer l) {
    ...
    log.log(Level.FINEST, "New udpSimple session");
  }
  ...
  public void handle(Event e) {
    ...
    if (e instanceof RegisterSocketEvent)
      ...
      try {
          log.log(Level.WARNING, ":handle: Unexpected event. Forwarding it...");
        e.go();
  ...
  private void handlePDUSize(MaxPDUSizeEvent e) {
    ...
    log.log(Level.FINEST, ":handlePDUSize ");
  ...
  private void handleDebug(Debug e) {
    ...
    log.log(Level.FINEST, ":handleDebug");
    
    int q = e.getQualifierMode();
    
    if (q == EventQualifier.ON) {
        log.log(Level.FINEST, "Ignored Debug event with qualifier ON.");
    } else if (q == EventQualifier.OFF) {
        log.log(Level.FINEST, "Ignored Debug event with qualifier OFF.");
  ...
  private void handleRegisterSocket(RegisterSocketEvent e) {
    ...
    log.log(Level.FINEST, ":handleRegisterSocket");
  ...
  private void handleMulticastInit(MulticastInitEvent e) {
    ...
    log.log(Level.FINEST, ":handleAppiaMulticastInit");
    ...
    if (!multicastReaders.containsKey(e.ipMulticast)) {
      ...
      try {
        ...
        log.log(Level.FINEST, ":handleAppiaMulticastInit: Socket Multicast created. Address: "  + e.ipMulticast);
        ...
        log.log(Level.FINEST, ":handleAppiaMulticastInit: Socket Multicast joined.");
      ...
      log.log(Level.FINEST, ":handleAppiaMulticastInit: Requested multicast socket already existed.");
    }
    ...
    try {
      ...
      e.init();
      e.go();
      log.log(Level.FINEST, ":handleAppiaMulticastInit: Returning multicastInit with error code: "+e.error);
      log.log(Level.FINEST, ":handleAppiaMulticastInit: Direction is "+(e.getDir() == Direction.DOWN? "DOWN":"UP"));
    } catch (AppiaEventException ex) {
      ex.printStackTrace();
    }
  }
  ...
  private void handleSendable(SendableEvent e) {
    ...
    log.log(Level.FINEST, ":handleSendable: "+e);
  ...
  private void handleChannelInit(ChannelInit e) {
    ...
    log.log(Level.FINEST, ":handleChannelInit from channel: "+e.getChannel().getChannelID());
  ...
  private void handleChannelClose(ChannelClose e) {
    ...
    log.log(Level.FINEST, ":handleChannelClose: Channel " + e.getChannel().getChannelID() + " closed");    
  ...
  private void formatAndSend(SendableEvent e) {
    ...
    try {
      ...
      if ((e.dest instanceof AppiaMulticast)
        ...
        for (int i = 0; i < dests.length; i++) {
          if (dests[i] instanceof InetSocketAddress) {
            ...
            if (debugFull)
              log.log(Level.FINEST, ":formatAndSend: Multicast emulation: " + dp.getLength() 
                  + " bytes datagram sent to " + dp.getAddress().getHostAddress() 
                  + " (port " + dp.getPort() + ")");
          } else
            log.log(Level.SEVERE, "UdpSimpleSession: Wrong destination address type in event " + e);
        }
      } else {
        ...
        if (debugFull)
          log.log(Level.FINEST, ":formatAndSend: "+dp.getLength()+" bytes datagram sent to "
      ...
      try {
        ...
        snd.go();
        log.log(Level.FINEST, ":formatAndSend: IOException when sending Datagram to socket. "
  ...
  class UdpSimpleReader implements Runnable {
    ...
    public void run() {
      ...
      logReader.log(Level.FINEST, "Reader running (Multicast="+(sock instanceof MulticastSocket)+").");
      ...
      while (running) {
        try {
          ...
          if (debugFull)
            logReader.log(Level.FINEST, ":run: PtP datagram received. Size = " + msg.getLength());
          
          if ((ignoreSource != null)
          && (ignoreSource.getPort() == msg.getPort())
          && (ignoreSource.getAddress().equals(msg.getAddress()))) {
            if (debugFull)
              logReader.log(Level.FINEST, ":run: Ignored Last received message");
          } else
...
//        	ste.printStackTrace();
    ...
    private void receiveFormatSend(DatagramPacket p) {
      ...
      try {
        ...
        if (debugFull) {
          logReader.log(Level.FINEST, ":receiveAndFormat: Reader, creating "+className+" event.");
        }
        ...
        if (msgChannel == null) {
        	if (debugFull)
        		logReader.log(Level.FINEST, this.getClass().getName()+
        ...
        if (debugFull)
          logReader.log(Level.FINEST, ":receiveAndFormat: "+msgChannel.getChannelID()+" ("+channelHash+")");
        ...
        if (Level.FINEST.equals(logReader.getLevel())) {
          ex.printStackTrace();
          logReader.log(Level.FINEST, "Exception catched while processing message from "+p.getAddress().getHostName()+":"+p.getPort()+". Continued operation.");
        }
      }
    }
  }
