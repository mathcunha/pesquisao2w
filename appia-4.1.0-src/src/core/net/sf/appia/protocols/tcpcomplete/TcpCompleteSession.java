...
public class TcpCompleteSession extends Session 
    ...
    private static Logger log = Logger.getLogger(TcpCompleteSession.class.getName());
...
//    otherSockets = new HashMap();
  ...
  protected void handleRegisterSocket(RegisterSocketEvent e){
      if(Level.FINEST.equals(log.getLevel()))
          log.log(Level.FINEST, "TCP Session received RegisterSocketEvent to register a socket in port "+e.port);
    ...
    if(ourPort < 0){
        if(e.port == RegisterSocketEvent.FIRST_AVAILABLE){
            try {
                ...
                log.log(Level.FINEST, "Exception when trying to create a server socket in First Available mode: "+ex);
            }
        }
            ...
            while(!done){
                ...
                try {
                    ...
                    log.log(Level.FINEST, "Exception when trying to create a server socket in Randomly Available mode: "+ex);
                } catch (IOException ex) {
                    log.log(Level.FINEST, "Exception when trying to create a server socket in Randomly Available mode: "+ex);
                }
            }
            ...
            try {
                ...
                log.log(Level.FINEST, "Exception when trying to create a server socket using the port: "+e.port+"\nException: "+ex);
            }
        }
    }
    if (ss != null) {
        ...
        if(Level.FINEST.equals(log.getLevel()))
            log.log(Level.FINEST, "TCP Session registered a socket in port "+ourPort);
  ...
  private void handleCloseSocket(CloseTcpSocket e) {
      ...
      if(existsSocket(otherReaders,dest)){
          ...
          if(Level.FINEST.equals(log.getLevel()))
              log.log(Level.FINEST, "Closing TCP socket for destination: "+dest);
      }
      else
          log.log(Level.FINEST, "Requested to close socket "+dest+" but the socket does not exist.");
  }
  ...
  private void handleChannelClose(ChannelClose e){
    ...
    if(channels.size() == 0){
        log.log(Level.WARNING, "No more channels. Cleaning sockets.");
