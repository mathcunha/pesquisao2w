...
public class ThroughputSession extends Session implements ManagedSession {
    ...
    private static Logger log = Logger.getLogger(ThroughputSession.class.getName());
    ...
    public void handle(Event event){
        if(event instanceof SendableEvent)
            ...
            try {
                log.log(Level.FINEST, "Forwarding unwanted event: "+event.getClass().getName());
                event.go();
    ...
    private void handleDebugTimer() {
        log.log(Level.FINEST, "Throughput going DOWN: Messages per second = "
                +msgPerSecondDown+" and bytes per second = "+bytesPerSecondDown);
        log.log(Level.FINEST, "Throughput going UP: Messages per second = "
