...
public class MessageIntegrityCheckerSession extends Session implements InitializableSession {
    ...
    private static Logger log = Logger.getLogger(MessageIntegrityCheckerSession.class.getName());
    ...
    public void handle(Event event){
        if(Level.FINEST.equals(log.getLevel()))
            log.log(Level.FINEST, "Received event on handle: "+event.getClass().getName()+" with direction "+(event.getDir()==Direction.UP? "UP":"DOWN"));
        if(!active){
            try {
                event.go();
        ...
        if(event instanceof SendableEvent){
            ...
            if(event.getDir() == Direction.DOWN){
                ...
                if(message.length() < HEADER_SIZE)
                    log.log(Level.WARNING, "Received event of type "+event.getClass().getName()+" with message length = "+message.length());
                else{
                    size = message.popInt();
                    if(message.length() != size){                        
                        log.log(Level.SEVERE, "Message integrity check has failed on tag [ "+tag+" ]. Message contains "
            ...
            log.log(Level.WARNING, "Received unexpected event of type "+event.getClass().getName()+". Forwarding it.");
            try {
                event.go();
            } catch (AppiaEventException e) {
                e.printStackTrace();
