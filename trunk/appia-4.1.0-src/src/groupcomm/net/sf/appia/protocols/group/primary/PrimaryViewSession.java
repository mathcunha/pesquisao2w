...
public class PrimaryViewSession extends Session implements InitializableSession, ManagedSession {
    ...
    private static Logger log = Logger.getLogger(PrimaryViewSession.class.getName());
    ...
    public void handle(Event event) {
        if (event instanceof View)
            ...
            if (Level.FINEST.equals(log.getLevel()))
                log.log(Level.SEVERE, "Received unexpected event: "+event);
    ...
    private void handleGroupSendable(GroupSendableEvent event) {
        if (blocked && event.getDir() == Direction.UP && this.view != null && event.view_id.equals(vs.id)){
            log.log(Level.FINEST, "I'm blocked and received message to be delivered in the current view");
            log.log(Level.FINEST, "Adding message to the pending buffer");
    ...
    private void handleView(View view) {
        ...
        if (vs == null) {
            ...
            if (isPrimary) {
                log.log(Level.FINEST, "My last view was primary");
                ...
                if (survivingMembers.length >= vsOld.view.length / 2 + 1) {
                    ...
                    log.log(Level.FINEST, "I'm still on a primary view");
                    ...
                    log.log(Level.FINEST, "Left the primary partition");
                    isPrimary = false;
                    wasPrimary = true;
    ...
    private void handleBlockOk(BlockOk ok) {
        log.log(Level.FINEST, "Received BlockOk");
        blocked = true;
        try {
            ok.go();
        } catch (AppiaEventException e) {
    ...
    private void handleProbeEvent(ProbeEvent event) {
        log.log(Level.FINEST, "Received ProbeEvent from "+event.orig);
        if (isPrimary) {
    ...
    private void handleKickEvent(KickEvent kick) {
        log.log(Level.FINEST, "Received KickEvent");
        leave(kick.getChannel());
    }
    
    private void handleDeliverViewEvent(DeliverViewEvent deliver) {
        log.log(Level.FINEST, "Received DeliverViewEvent");
    ...
    private void deliverView() {
        log.log(Level.FINEST, "Delivering Primary View");
    ...
    private void kick(Channel ch, int dest) {
        log.log(Level.FINEST, "Kicking process "+dest+"...");
        try {
    ...
    private void leave(Channel ch) {
        ...
        log.log(Level.FINEST, "Leaving group...");
        try {
    ...
    public void setParameter(String parameter) throws AppiaManagementException {
        if(parameter.equals(SET_PRIMARY)){
            if(blocked && view == null){
                log.log(Level.WARNING, "Process set to Primary by Management.");
                primaryProcess = true;
            }
                ...
                log.log(Level.WARNING, "Management instruction "+parameter+" ignored.");
            }
        }
        else 
...
//                        System.out.println("->"+dest);
            ...
            log.log(Level.WARNING, "Management instruction "+SET_PRIMARY+" ignored.");
    ...
    public Object invoke(String action, MBeanOperationInfo info, Object[] params, String[] signature) 
        ...
        log.log(Level.FINEST, "Calling action: "+action);
