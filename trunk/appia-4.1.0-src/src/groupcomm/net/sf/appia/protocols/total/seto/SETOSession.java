...
public class SETOSession extends Session implements InitializableSession {
	...
	private static Logger log = Logger.getLogger(SETOSession.class.getName());
...
//	private Channel channel = null;
	...
	public void handle(Event event){
		if(event instanceof ChannelInit)
			...
			log.log(Level.WARNING, "Got unexpected event in handle: "+event+". Forwarding it.");
			try {
				event.go();
	...
	private void handleChannelClose(ChannelClose close) {
		log.log(Level.WARNING, "Channel is closing!");
		try {
			close.go();
		} catch (AppiaEventException e) {
			e.printStackTrace();
	...
	private void handleBlockOk(BlockOk ok) {
		log.log(Level.FINEST, "The group is blocked.");
		log.log(Level.FINEST, "Impossible to send messages. Waiting for a new View");
		isBlocked = true;
...
//            deliverPendingView();
		...
		log.log(Level.FINEST, vs.toString());
		log.log(Level.FINEST, ls.toString());
		log.log(Level.FINEST, "NEW VIEW: My rank: "+ls.my_rank+" My ADDR: "+vs.addresses[ls.my_rank]);
	}
...
//                dest[i] = vs.getRank(survivors[i]);
    ...
    private void deliverPendingView() {
        ...
        if(!pendingMessages.isEmpty()){
            log.log(Level.FINEST, "Delivering "+pendingMessages.size()+" pending messages");
	...
	private void handleGroupSendable(GroupSendableEvent event) {
		//log.log(Level.FINEST, "------------> " + vs.addresses[ls.my_rank] + " Received from "+vs.addresses[event.orig]);
		// events from the application
		if(event.getDir() == Direction.DOWN){
			if(isBlocked){
			    log.log(Level.WARNING, "Received event while blocked:"+event.getClass().getName()+" from session: "+
		...
		// events from the network
            ...
            if (pendingView != null) {
                log.log(Level.FINEST, "Received GroupSendableEvent but still haven't delivered pending view");
                log.log(Level.FINEST, "Buffering event for future delivery");
	...
	private void reliableDATAMulticast(GroupSendableEvent event, long msgDelay) {
		...
		log.log(Level.FINEST, "Sending DATA message from appl. Rank="+ls.my_rank+" SN="+sendingLocalSN+" Delay="+msgDelay);
		try {
			event.go();
		} catch (AppiaEventException e) {
			e.printStackTrace();
	...
	private void reliableDATADeliver(GroupSendableEvent event){
		...
		log.log(Level.FINEST, "Received DATA message: "+header.id+":"+header.sn+" timestpamp is "+timeProvider.currentTimeMillis());
	...
	private void handleTimer(SETOTimer timer) {
		if (timer.vid.equals(vs.id)) {
			...
			log.log(Level.FINEST, ls.my_rank+": received timer on "+now);
			deliverOptimistic(timer.container);
		}
		else
			log.log(Level.FINEST, ls.my_rank+": received SETOTimer from a previous view... discarding!");
	}
	
	private void deliverOptimistic(ListContainer container) {
		if (!O.contains(container)) {
			log.log(Level.FINEST, "Delivering optimistic message.");
			try {
			...
			if(coordinator() && !isBlocked) {
				log.log(Level.FINEST, "I'm the coordinator. Sending message to order");
				globalSN++;
	...
	private void reliableSEQMulticast(ListContainer container) {
		...
		try {
			...
			for (int i = 0; i < lastOrderList.length; i++)
				msg.pushLong(lastOrderList[i]);
			log.log(Level.FINEST, "Sending SEQ message. Rank="+ls.my_rank+" Header: "+header);
			event.go();
	...
	private void handleSequencerMessage(SeqOrderEvent message) {
		log.log(Level.FINEST, "Received SEQ message from "+message.orig+" timestamp is "+timeProvider.currentTimeMillis());
		if(message.getDir() == Direction.DOWN)
			log.log(Level.SEVERE, "Wrong direction (DOWN) in event "+message.getClass().getName());
		else
	...
	private void reliableSEQDeliver(SeqOrderEvent event) {
		...
		log.log(Level.FINEST, "["+ls.my_rank+"] Received SEQ message "+header.id+":"+header.sn+" timestamp is "+timeProvider.currentTimeMillis());
		...
		log.log(Level.FINEST, "Received SEQ from "+event.orig+" at time "+timeProvider.currentTimeMillis());
	...
	private void deliverRegular() {
	    for (ListIterator li = S.listIterator(); li.hasNext(); ) {
	        ...
	        if (Level.FINEST.equals(log.getLevel())) {
	            log.log(Level.FINEST, "Message in order with SN="+(localSN+1)+" -> "+orderedMsg);
	            log.log(Level.FINEST, "Messages in S {");
	            listOrderedMessage();
	        ...
	        if (msgContainer != null && !hasMessage(orderedMsg,G)) {
	            log.log(Level.FINEST, "["+ls.my_rank+"] Delivering regular "+msgContainer.header.id+":"+msgContainer.header.sn+" timestamp "+timeProvider.currentTimeMillis());
	            try {
	            ...
	            if (pendingView == null) {
	                // ADJUSTING DELAYS
	                log.log(Level.FINEST, ls.my_rank+": Adjusting delays...");
	                long _final = orderedMsg.time;
	                long _fast = msgContainer.header.getTime();
	                int _sender = msgContainer.header.id;
	                if(lastsender != -1){
	                    log.log(Level.FINEST, "continuing adjusting the delays!");
	                    log.log(Level.FINEST, "_final:"+_final+" | lastfinal:"+lastfinal+" | _fast:"+_fast+" | lastfast:"+lastfast);
	                    long delta = (_final - lastfinal) - (_fast - lastfast);
	                    log.log(Level.FINEST, "DELTA: "+delta);
	                    if(delta > 0) {
	                        log.log(Level.FINEST, "adjust("+lastsender+","+_sender+","+delta+")");
	                        adjust(lastsender,_sender,delta);
	                    }
	                    else if (delta < 0) {
	                        log.log(Level.FINEST, "adjust("+_sender+","+lastsender+","+delta+")");
	    ...
	    log.log(Level.FINEST, "DeliverRegular finished.");
	}

	private void handleUniformTimer(UniformTimer timer) {
	    //log.log(Level.FINEST, "Uniform timer expired. Now is: "+timeProvider.currentTimeMillis());
		if (!isBlocked && newUniformInfo && timeProvider.currentTimeMillis() - timeLastMsgSent >= UNIFORM_INFO_PERIOD) {
			//log.log(Level.FINEST, "Last message sent was at time "+timeLastMsgSent+". Will send Uniform info!");
	...
	private void handleUniformInfo(UniformInfoEvent event) {
		log.log(Level.FINEST, "Received UniformInfo from "+event.orig+". Uniformity information table now is: ");
		Message msg = event.getMessage();
		long[] uniformInfo = new long[vs.view.length];
		for (int i = uniformInfo.length; i > 0; i--)
			uniformInfo[i-1] = msg.popLong();
		...
		if (Level.FINEST.equals(log.getLevel()))
			for (int i = 0; i < lastOrderList.length; i++)
				log.log(Level.FINEST, "RANK :"+i+" | LAST_ORDER: "+lastOrderList[i]);
		deliverUniform();
	}
	...
	private void deliverUniform() {
		log.log(Level.FINEST, "Trying to deliver FINAL messages!");
		...
		while (it.hasNext()) {
			...
			if (isUniform(nextMsg.header)) {
				...
				log.log(Level.FINEST, "Delivering message: "+msgContainer.event);
				log.log(Level.FINEST, "["+ls.my_rank+"] Delivering final "+msgContainer.header.id+":"+msgContainer.header.sn+" timestamp "+timeProvider.currentTimeMillis());
				try {
	...
	private void dumpPendingMessages() {
		...
		while((container = getNextDeterministic()) != null){
			if(Level.FINEST.equals(log.getLevel())){
				log.log(Level.FINEST, "Message in deterministic order with SN="+(localSN+1)+" -> "+container);
			}
			SEQHeader header = new SEQHeader(container.header.sender(), container.header.sn(), ++lastOrderList[ls.my_rank]);
            lastOrderList[ls.my_rank] = header.order;
			S.add(new ListSEQContainer(header,timeProvider.currentTimeMillis()));
			log.log(Level.FINEST, "Resending message to Appl: "+container.event);
	...
	private ListSEQContainer listOrderedMessage(){
		for (ListIterator li=S.listIterator(); li.hasNext();){
			ListSEQContainer cont = (ListSEQContainer) li.next();
			log.log(Level.FINEST, "Element: "+cont.header);
		} 
		return null;
	}
