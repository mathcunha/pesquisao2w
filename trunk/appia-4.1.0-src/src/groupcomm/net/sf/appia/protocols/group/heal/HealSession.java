...
public class HealSession extends Session implements InitializableSession {
    private static Logger log = Logger.getLogger(HealSession.class.getName());
    ...
    private void handleGossipOrHello(SendableEvent ev) {
        ...
        if(vs == null){
            log.log(Level.FINEST, "Received gossip or hello but didn't received first view.");
            return;
        }
        ...
        if (!vs.group.equals(remote_group)) {
            log.log(Level.FINEST, "Received gossip or hello of other group. Ignoring it.");
            return;
        }

        if (vs.id.equals(remote_id)) {
            log.log(Level.FINEST, "Received gossip or hello from my current view. Ignoring it.");
            return;
        }
        ...
        for (i=0 ; i < vs.previous.length ; i++) {
            if (vs.previous[i].equals(remote_id)) {
                log.log(Level.FINEST, "Received gossip or hello from my old view. Ignoring it.");
                return;
            }
        }

        if (vs.getRankByAddress((InetSocketAddress)ev.source) >= 0) {
            log.log(Level.FINEST, "Received gossip of other alive member of my group (possibly an old one). Ignoring it.");
            return;
        }

        if (detectedViews.contains(remote_id)) {
            log.log(Level.FINEST, "Received gossip or hello from an already detected concurrent view. Ignoring it.");
            return;
        }

        if (debugFull)
            log.log(Level.FINEST, "Detected valid concurrent view (id="+remote_id.toString()+" source="+((InetSocketAddress)ev.source).toString()+"). Sending warning.");
        ...
        try {
            ...
            if (debugFull)
                ex.printStackTrace();
            log.log(Level.WARNING, "Unable to send ConcurrentViewEvent");
        }
    }
    ...
    private void handleConcurrentView(ConcurrentViewEvent event) {
        ...
        if (detectedViews.contains(event.id)) {
            log.log(Level.FINEST, "Received ConcurrentViewEvent of an already detected view. Ignoring it.");
            return;
        }

        if (!ls.am_coord) {
            log.log(Level.FINEST, "Received ConcurrentViewEvent but i am not the coordinator.");
            detectedViews.add(event.id);
            return;
        }

        if (debugFull)
            log.log(Level.FINEST, "Received valid concurrent view detection (id="+event.id+" source="+event.addr+"). Resending it.");
        ...
        try {
            ...
            ex.printStackTrace();
            log.log(Level.WARNING, "Impossible to reverse and resend a received ConcurrentViewEvent");
        }
    }
    ...
    private void handleOtherViews(OtherViews ev) {
        ...
        if (!ls.am_coord) {
            log.log(Level.FINEST, "Received other view warning but i am not the coordinator");
            return;
        }
        ...
        if (now-last_hello > hello_min_time) {
            ...
            if (debugFull)
                log.log(Level.FINEST, "Sent Hello due to OtherViews ("+ev.other_addr.toString()+")");
        }
    }
    ...
    private void sendHello(Channel channel, Object addr) {
                ...
                if(other_addrs != null){
                    for(int i=0 ; i < other_addrs.length ; i++){
                        if(vs.getRankByAddress((InetSocketAddress)other_addrs[i]) == -1){
                            if(debugFull)
                                log.log(Level.FINEST, "Sending hello to "+other_addrs[i]);
        ...
        try {
            ...
            ex.printStackTrace();
            log.log(Level.WARNING, "Impossible to send HelloEvent");
        }
    }

    private void sendGossip(Channel channel) {
        if (debugFull)
            log.log(Level.FINEST, "Sending Gossip to Server");
        ...
        try {
            ...
            ex.printStackTrace();
            log.log(Level.WARNING, "Impossible to send GossipOutEvent");
        }
    }
