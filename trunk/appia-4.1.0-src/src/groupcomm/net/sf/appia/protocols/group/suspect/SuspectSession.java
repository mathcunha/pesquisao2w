...
public class SuspectSession extends AbstractSensorSession implements InitializableSession, ManagedSession {
    private static Logger log = Logger.getLogger(SuspectSession.class.getName());
    ...
    private void handleGroupSendableEvent(GroupSendableEvent ev) {
        ...
        if (ev.getDir() == Direction.DOWN) {
            ...
            if (debugFull)
                log.log(Level.FINEST, "Sent msg ("+ev+") in round "+round);
        } else {
            last_recv[ev.orig]=round;
            if (debugFull)
                log.log(Level.FINEST, "Recv msg from "+ev.orig+" in round "+round);
        }
    ...
    private void handleSuspect(Suspect ev) {
        ...
        log.log(Level.FINEST, "Received Suspect from "+ev.orig);
        if (ev.getDir() == Direction.UP) {
            if ( ls.failed[ev.orig] ) {
                log.log(Level.FINEST, "Invalid (failed) message source");
                return;
            }

            ev.failed=ArrayOptimized.popArrayBoolean(ev.getMessage());
            if(Level.FINEST.equals(log.getLevel())){
                log.log(Level.FINEST, "Failed on suspect: ");
                for(int i=0; i<ev.failed.length; i++)
                    log.log(Level.FINEST, "Member "+i+" failed="+ev.failed[i]);
            }

        }

        if (ev.failed[ls.my_rank]) {
            log.log(Level.FINEST, "i am not failed !!");
            return;
        }
        ...
        for (i=0 ; i < ev.failed.length ; i++) {
            if (ev.failed[i] && !ls.failed[i]) {
                ...
                new_failed[i]=true;
                if(Level.FINEST.equals(log.getLevel()))
                    log.log(Level.FINEST, "Member "+i+" has failed. Setting its flag in the array.");
            }
        }
    ...
    private void handleSuspectTimer(SuspectTimer ev) {
        if(ev.getPeriod() != suspect_sweep){
            ...
            try {
                ...
                ex.printStackTrace();
                log.log(Level.SEVERE, "appia:group:SuspectSession: impossible to set SuspectTimer, SuspectSession will be idle");
            }
            return;
        }
        ...
        for (i=0 ; i < last_recv.length ; i++) {
            if (i != ls.my_rank) {
                if ( (round-last_recv[i] >= rounds_idle) && !ls.failed[i] ) {
                    ...
                    log.log(Level.FINEST, "Suspected "+i+" because it passed "+(round-last_recv[i])+" rounds of "+suspect_sweep+" milliseconds since last reception");
                }
            }
        }
        ...
        if (round > last_recv[ls.my_rank]) {
            ...
            last_recv[ls.my_rank]=round;
            if(debugFull)
                log.log(Level.FINEST, "Sent Alive in round "+round);
        }

        if (debugFull)
            log.log(Level.FINEST, "Ended round "+round+" at "+ev.getChannel().getTimeProvider().currentTimeMillis()+" milliseconds");
    ...
    private void handleFIFOUndeliveredEvent(FIFOUndeliveredEvent ev) {
        ...
        if (!vs.group.equals(event.group)) {
            log.log(Level.FINEST, "Ignored FIFOUndelivered due to wrong group");
            return;
        }

        if (!vs.id.equals(event.view_id)) {
            log.log(Level.FINEST, "Ignored FIFOUndelivered due to wrong view id");
            return;
        }
        ...
        if (event.dest instanceof InetSocketAddress)
            ...
            log.log(Level.FINEST, "Received FIFOUndelivered with unknown destination address. Ignoring it.");
    }
    ...
    private void handleSuspectedMember(SuspectedMemberEvent ev){
        ...
        if(!ev.getGroup().equals(vs.group) && !ev.getViewID().equals(vs.id)){
            log.log(Level.FINEST, "SuspectedMemberEvent from another group or view. Discarding it");
            return;
        }
        if(ev.getSuspectedMember() >= 0)
            processUndelivered(ev.getSuspectedMember(),ev.getChannel());        
        else
            log.log(Level.FINEST, "SuspectedMemberEvent didn't contain a valid view member.");
    }
    ...
    private void undelivered(InetSocketAddress addr, Channel channel) {
        ...
        if ((rank=vs.getRankByAddress(addr)) >= 0) {
            ...
            log.log(Level.FINEST, "Undelivered didn't contain a current view member");
    }
    ...
    private void processUndelivered(int rank, Channel channel) {
        if (!ls.failed[rank]) {
            ...
            log.log(Level.FINEST, "Suspected member "+rank+" due to Undelivered");
        }
    }
    ...
    private void sendSuspect(boolean[] failed, Channel channel) {
        try {
            ...
            ex.printStackTrace();
            log.log(Level.WARNING, "Impossible to send Suspect");
        }
    }
    ...
    private void sendAlive(Channel channel) {
        ...
        try {
            ...
            ex.printStackTrace();
            log.log(Level.WARNING, "Impossible to send alive");
        }
    }
