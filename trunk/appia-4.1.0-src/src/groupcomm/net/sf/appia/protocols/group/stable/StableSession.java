...
public class StableSession extends Session {
    private static Logger log = Logger.getLogger(StableSession.class.getName());
    ...
    private void handleGroupSendableEvent(GroupSendableEvent ev) {
        ...
        if (ev instanceof Suspect){
            log.log(Level.FINEST, "Received unexpected Suspect event. Ignoring it");
            return;
        }
        ...
        if (ev.getDir() == Direction.DOWN) {
            ...
            if (seq-last_gossip > gossip_interval) {
                ...
                last_gossip=seq;
                if (debugFull)
                    log.log(Level.FINEST, "Placing gossip information in "+(ev instanceof StableGossip ? "StableGossip" : "message")+" with number "+last_gossip);
            } else {
            ...
            if (ev.getMessage().popBoolean()) {
                ...
                if (debugFull)
                    log.log(Level.FINEST, "Received gossip information from "+ev.orig+" (doClean="+doClean+")");
            } else {
                seqno=popSeq(ev.getMessage(), table[ls.my_rank][ev.orig]);
            }

            if (debugFull)
                log.log(Level.FINEST, "Received "+seqno+" from "+ev.orig);
            ...
            if (!(ev instanceof StableGossip)) {
                ...
                try {
                    ...
                    table[ls.my_rank][ev.orig]--;
                    ex.printStackTrace();
                    log.log(Level.WARNING, "Event ("+ev.getClass().getName()+") discarded because it was impossible to store");
                }
    ...
    private void handleRetransmit(Retransmit ev) {
        ...
        while (lo <= hi) {
            if ((info=storage.get(rank,lo)) == null) {
                log.log(Level.FINEST, "I don't have message "+lo+" from "+rank+" as requested");
                return;
            }
    ...
    private void handleRetransmission(Retransmission ev) {
        ...
        if (seqno != table[ls.my_rank][orig]+1) {
            log.log(Level.FINEST, "Received Retransmission of message "+seqno+" already received (strange but ...). Discarding retrasmission.");
            return;
        }

        if (debugFull) 
            log.log(Level.FINEST, "Received Retransmission from "+ev.orig+" of message "+seqno+" sent by "+orig+" in event of class "+name);
        ...
        try {
            ...
            ex.printStackTrace();            
            log.log(Level.WARNING, "Impossible to create/send retransmited event");
    ...
    private void suspect(GroupSendableEvent ev, long received) {
        ...
        try {
            ...
            sme.go();
            log.log(Level.FINEST, "Suspecting member "+ev.orig+". Notification sent to the failure detector.");
        } catch (AppiaEventException ex) {
            ex.printStackTrace();
        }
    }
    ...
    private void retransmit(int dest, int rank, long lo, long hi, Channel channel) {
        if (debugFull)
            log.log(Level.FINEST, "Requesting retransmisson of messages ["+lo+","+hi+"] from member "+rank+" to alive member "+dest);
        try {
