...
public class GroupBottomSession extends Session {
    private static Logger log = Logger.getLogger(GroupBottomSession.class.getName());
    ...
    private void handleUpGroupSendableEvent(GroupSendableEvent ev) {
        ...
        try {
            ...
            if (groupHash != groupHashCode) {
                log.log(Level.WARNING, "discarded event ("+ev+") due to bad \"group\"");
                return;
            }
            ...
            if (viewHash != viewHashCode) {
                ...
                if (isNext) {
                    if (debugFull)
                        log.log(Level.FINEST, "buffering event from possible next view (hash="+viewHash+").");
                ...
                for (int i=0 ; i < vs.previous.length ; i++) {
                    if (viewHash == vs.previous[i].hashCode()) {
                        log.log(Level.FINEST, "discarded event because it belonged to previous view");
                        return;
                    }
                }
                ...
                log.log(Level.FINEST, "received event from same \"group\", but different \"view id\". Sent OtherView. "+ ev.getClass().getName());
            ...
            if ((ev.orig < 0) || (ev.orig >= vs.view.length) || (ev.orig == ls.my_rank)) {
                log.log(Level.FINEST, "Event discarded due to bad origin "+ev.orig);
                return;
            }
            ...
            if (ev instanceof Send) {
                ...
                if (!bitset.get(ls.my_rank)) {
                    log.log(Level.FINEST, "send event discarded because it wasn't for me ("+ev+")");
                    return;
                }
            }
            ...
            if (debugFull)
                log.log(Level.FINEST, "Received message "+ev+" from "+ev.orig);

            try {
                ev.go();
            } catch (AppiaEventException ex) {
                ex.printStackTrace();
                log.log(Level.WARNING, "up event discarded");
            }
        } catch (MessageException ex) {
            log.log(Level.WARNING, "Event (\""+ev.getClass().getName()+"\") discarded because i couldn't read headers.");
            return;
        }
    }
    ...
    private void handleDownGroupSendableEvent(GroupSendableEvent ev) {
        if ((vs == null) || (vs.view.length == 1)) {
            log.log(Level.FINEST, "Trying to send to somenone but i'm the only one. Discarding Event");
            return;
        }
        ...
        if (isSend) {
            ...
            for (int i=0 ; i < dests.length ; i++) {
                if((dests[i] >= vs.addresses.length) || (dests[i] < 0))
                    log.log(Level.FINEST, "Invalid destination rank ("+dests[i] +") in Send. Event ("+ev.getClass().getName()+ ") Discarded.");
                else
                    bitset.set(dests[i]);
            }
        }
        ...
        if (debugFull)
            log.log(Level.FINEST, "Sending message (isSend="+isSend+") "+ev);
    ...
    private void handleView(View ev) {
        ...
        if (debugFull)
            log.log(Level.FINEST, "Received new view: group="+groupHashCode+" view="+viewHashCode);
        ...
        while ((e=buffer.get()) != null) {
            if (Level.FINEST.equals(log.getLevel()))
                log.log(Level.FINEST, "Debuffering event "+e);
    ...
    private void handleGroupEvent(GroupEvent ev) {
        if (!vs.group.equals(ev.group) || !vs.id.equals(ev.view_id)) {
            log.log(Level.FINEST, "event (\""+ev.getClass().getName()+"\") going ("+ev.getDir()+") discarded due to bad group or view_id");
            return;
        }
    ...
    private void sendOtherViews(int state, Object other_addr, Object multicast_addr, Channel channel) {
        try {
            ...
            ex.printStackTrace();
            log.log(Level.WARNING, "Impossible to send OtherViews ("+state+")");
        }
    }
