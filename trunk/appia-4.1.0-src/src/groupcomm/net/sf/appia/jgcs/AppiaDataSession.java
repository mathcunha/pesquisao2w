...
public class AppiaDataSession extends AbstractDataSession {
	...
	private static Logger logger = Logger.getLogger(AppiaDataSession.class.getName());
	private static Logger workerlog = Logger.getLogger(PullPushWorker.class.getName());
	...
	public AppiaDataSession(AppiaProtocol proto, AppiaGroup group, Mailbox<Event> mbox, 
		...
		logger.log(Level.FINEST, "Number of channels: "+channels.size());
		for(Channel ch : channels){
			logger.log(Level.FINEST, "Channel: "+ch.getChannelID());
	...
	private void sendMessage(Message msg, Service service, Object cookie, SocketAddress destination, 
		...
		if(service != null){
			if(Level.FINEST.equals(logger.getLevel()))
				logger.log(Level.FINEST, "Service on send: "+((AppiaService)service).getService());
		...
		if(Level.FINEST.equals(logger.getLevel()))
			logger.log(Level.FINEST, "Message "+msg+" delivered to the Appia channel with service "+service);
	}
	...
	class PullPushWorker implements Runnable {
		...
		public void run() {
		    ...
		    try{
		        while(running.get()){
		            workerlog.log(Level.FINEST, "before receive");
		            event = mailbox.removeNext();
		            if(event == null){
		                if(Level.FINEST.equals(workerlog.getLevel()))
		                    workerlog.log(Level.FINEST, "Received null event from Appia mailbox");
		                continue;
		            }
		            if(Level.FINEST.equals(workerlog.getLevel()))
		                workerlog.log(Level.FINEST, "after receive: "+event);
		            if(event instanceof JGCSGroupEvent || event instanceof JGCSSendEvent){
		                ...
		                try {
		                    ...
		                    workerlog.log(Level.FINEST, "Received message but I'm not in the group: "+event,e1);
		                ...
		                if(Level.FINEST.equals(workerlog.getLevel()))
		                    workerlog.log(Level.FINEST, "Delivering message: "+msg);
		                final Object ctx = notifyMessageListeners(msg);
		                if(ctx != null){
		                    servicesMap.put(msg,ctx);
		                    if(Level.FINEST.equals(workerlog.getLevel()))
		                        workerlog.log(Level.FINEST, "Received context for this message. Adding to the services map:\nMessage:: "+msg+
		                ...
		                if(Level.FINEST.equals(workerlog.getLevel()))
		                    workerlog.log(Level.FINEST, "Delivering message coming from outside of the group: "+msg);
		                Object ctx = notifyMessageListeners(msg);
		                if(ctx != null){
		                    logger.log(Level.WARNING, "The Service feature is not supported for this kind of messages. Ignoring it.");
		                ...
		                if(Level.FINEST.equals(logger.getLevel()))
		                    workerlog.log(Level.FINEST, "Received group event.");
		        ...
		        workerlog.log(Level.WARNING, "Exception in the worker Thread: "+rte+"\nwhile processing event "+event.toString());
		        rte.printStackTrace();
		...
		private void handleServiceEvent(ServiceEvent event) {
			if(Level.FINEST.equals(workerlog.getLevel()))
				workerlog.log(Level.FINEST, "Received service event from Appia "+event);
			...
			if(event instanceof SETOServiceEvent)
				...
				if(Level.FINEST.equals(workerlog.getLevel()))
					workerlog.log(Level.FINEST, "Received unrecognized Service event from Appia: "+event);
				return;
			}
			Object context = servicesMap.get(event.getMessageID());
			if(Level.FINEST.equals(workerlog.getLevel()))
				workerlog.log(Level.FINEST, "Application context for message="+event.getMessageID()+" is ctx="+context);
			if(context != null){
				notifyServiceListeners(context,currentService);
				if(isLastService){
					context = servicesMap.remove(event.getMessageID());
					if(Level.FINEST.equals(workerlog.getLevel()))
						workerlog.log(Level.FINEST, "Last service notified. Removing entry from hashtable --> Context removed: "+context+
