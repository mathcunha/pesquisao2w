...
public class SimpleTOPSession extends Session implements InitializableSession {
	...
	private static Logger logger = Logger.getLogger(SimpleTOPSession.class.getName());
	...
	private void handleMessageSender(MessageSender sender) {
		...
		// UP should not happen
			...
			logger.log(Level.WARNING, "MessageSender event arrived from a bottom layer. This kind of events should appear only from the application.");
		}
	}
	...
	private void handleSendableEvent(JGCSSendableEvent event) {
		...
		// event from application
			...
			logger.log(Level.WARNING, "JGCSSendable event arrived from the application. This kind of events should appear only from the network.");
		}
//			if(!receivedRSE){
//				eventsPending.add(event);
//				return;
