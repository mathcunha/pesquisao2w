...
public class TOPSession extends Session implements InitializableSession {
	...
	private static Logger logger = Logger.getLogger(TOPSession.class.getName());
	...
	public void handle(Event event){
		if(Level.FINEST.equals(logger.getLevel()))
			logger.log(Level.FINEST, "TOP session received event "+event+" "+(event.getDir() == Direction.DOWN? "Down":"Up")
	...
	private void handleMessageSender(MessageSender sender) {
		if(Level.FINEST.equals(logger.getLevel()))
			logger.log(Level.FINEST, "Received Message "+sender.getMessage()+" from the DataSession.");
		...
		if(isBlocked){
            ...
            logger.log(Level.WARNING, "The group is blocked. Message "+sender.getMessage()+" added to pending events.");
			return;
		}
		try {
			event.go();
		...
		if(Level.FINEST.equals(logger.getLevel()))
			logger.log(Level.FINEST, "Message "+sender.getMessage()+" Forwarded to the Channel "+event.getChannel().getChannelID());
	}
	...
	private void handleBlock(BlockOk e) {
		if(leaveChannel != null){
			if(Level.FINEST.equals(logger.getLevel()))
				logger.log(Level.FINEST, "Leave latch is present. Channel is closing. Forwarding BlockOk without delivering it to the Application.");
			try {
				e.go();
			} catch (AppiaEventException e1) {
				logger.log(Level.FINEST, "Error forwarding event: "+e1.getMessage());
			}
		}
		else{
			if(numberOfBlocks<numberOfChannels)
				...
				if(Level.FINEST.equals(logger.getLevel()))
					logger.log(Level.FINEST, "Delivering BlockOk to the Application.");
				mailbox.add(e);
				numberOfBlocks = 1;
			}
		}
	...
	private void handleRegisterSocket(RegisterSocketEvent e) {		
		if (e.error) {
			if(e.getErrorCode() == RegisterSocketEvent.RESOURCE_ALREADY_BOUND_ERROR)
				logger.log(Level.WARNING, "The requested resource is already available.");
			else{
				logger.log(Level.SEVERE, "Impossible to register socket.");
			}
		}
		
		myAddress = new InetSocketAddress(e.localHost, e.port);
		receivedRSE = true;
		logger.log(Level.FINEST, "Socket Registered using the address: "+myAddress);
	...
	private void handleMulticastInit(MulticastInitEvent event) {
		if(event.error){
			logger.log(Level.WARNING, "Impossible to register multicast address. Using Point to Point");
		}		
		// this is done only once and when there are conditions to initialize the group.
		// the tests to ensure this are made inside the method
		sendGroupInit(event.getChannel());
	...
	private void handleChannelInit(ChannelInit e) {
		if(gossips == null || gossips.length == 0){
			logger.log(Level.SEVERE, "Received channel init but no gossip is configured.");
