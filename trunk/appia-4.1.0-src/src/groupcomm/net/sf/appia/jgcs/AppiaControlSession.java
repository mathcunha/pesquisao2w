...
public class AppiaControlSession extends AbstractBlockSession {
	...
	private static Logger logger = Logger.getLogger(AppiaControlSession.class.getName());
	...
	public void blockOk() throws NotJoinedException, JGCSException{
		...
		if(am.blockEvent != null) {
			...
			logger.log(Level.FINEST, "Block released to the Appia channel");
			am.blockEvent = null;
		}
	}
	...
	public void leave() throws ClosedSessionException, JGCSException{
		if(Level.FINEST.equals(logger.getLevel()))
			logger.log(Level.FINEST, "Leaving from Appia Session... [ControlSession.leave()]");
	...
	void notifyListeners(GroupEvent event){
		...
		try {
			...
			logger.log(Level.FINEST, "Exception on notifyListeners: "+e1);
		...
		if(event instanceof BlockOk){
			if(Level.FINEST.equals(logger.getLevel()))
				logger.log(Level.FINEST, "Received block event.");
			...
			if(currentMembership == null){
				if(Level.FINEST.equals(logger.getLevel()))
					logger.log(Level.FINEST, "Processing first view.");
				...
				if(Level.FINEST.equals(logger.getLevel()))
					logger.log(Level.FINEST, "Processing next view.");
				incomingMembership =new AppiaMembership((View) event);
			...
			if(Level.FINEST.equals(logger.getLevel()))
				logger.log(Level.FINEST, "currentMembership:"+incomingMembership.addresses);
		}
		else{
			logger.log(Level.FINEST, "Received wrong GroupEvent: "+event);
		}
	}
