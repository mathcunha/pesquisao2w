...
public class NakFifoMulticastSession extends Session implements InitializableSession {
    private static Logger log = Logger.getLogger(NakFifoMulticastSession.class.getName());
  ...
  public void handle(Event event) {
    ...
    log.log(Level.WARNING, "Unwanted event (\""+event.getClass().getName()+"\") received. Continued...");
  ...
  private void handleChannelInit(ChannelInit ev) {
    ...
    log.log(Level.FINEST, "Params: "+ev.getChannel().getChannelID()+"\n\tTIMER_PERIOD="+param_TIMER_PERIOD+
  ...
  private void handleChannelClose(ChannelClose ev) {
    ...
    if (ev.getChannel() == timerChannel) {
      ...
      if (timerChannel != null)
        log.log(Level.FINEST, "Sent new timer in channel "+timerChannel.getChannelID());
      else
        log.log(Level.WARNING, "Unable to send timer. Corret operation is not garanteed");
    }
  }
  ...
  */
  ...
  private void handleSendable(SendableEvent event) {
    if (event.getDir() == Direction.UP) {
      ...
      if ((flags & MessageUtils.IGNORE_FLAG) != 0) {
        log.log(Level.FINEST, "Received message with ignore flag. Ignoring.");
        try { event.go(); } catch (AppiaEventException ex) { ex.printStackTrace(); }
        return;
      }
      ...
      if ((seq=utils.popSeq(event.getMessage(),peer.last_msg_delivered,false)) < 0) {
        log.log(Level.FINEST, "Problems reading sequence number discarding event "+event+" from "+event.source.toString());
        return;
      }
    ...
    if (event.getDir() == Direction.DOWN) {
      ...
      if ((event.dest instanceof InetSocketAddress) && (((InetSocketAddress)event.dest).getAddress().isMulticastAddress())) {
        log.log(Level.FINEST, "Destination is a IP Multicast address. Ignored.");
      ...
      try {
        ...
        ex.printStackTrace();
        log.log(Level.WARNING, "To mantain coerence, sending undelivered.");
    ...
    log.log(Level.WARNING, "Direction is wrong. Discarding event "+event);
  }
  
  private void handlePing(PingEvent ev) {
    if (ev.getDir() != Direction.UP) {
      log.log(Level.WARNING, "Discarding Ping event due to wrong diretion.");
      return;
    }
    ...
    if (peer == null) {
    	log.log(Level.FINEST, "Received Ping from unknown peer ("+ev.source+"). Ignoring confirm.");
    	ev.getMessage().discard(MessageUtils.SEQ_SIZE);
    } else {
    	long confirmed;
    	if ((confirmed=utils.popSeq(ev.getMessage(),peer.last_msg_confirmed,false)) < 0) {
    		log.log(Level.FINEST, "Problems reading confirm sequence number from "+ev.source);
    	} else {
  ...
  private void handleNack(NackEvent ev) {
    ...
    if ((first=ev.getMessage().popLong()) < 0) {
      log.log(Level.FINEST, "Ignoring Nack due to wrong first seq number.");
      return;
    }
    if ((last=ev.getMessage().popLong()) < 0) {
      log.log(Level.FINEST, "Ignoring Nack due to wrong last seq number.");
      return;
    }
    if (first > last) {
      log.log(Level.FINEST, "Ignoring Nack due to wrong seq numbers (first="+first+",last="+last+",confirmed="+peer.last_msg_confirmed+").");
      return;
    }
    
    if ((first < first_msg_sent) || (last > last_msg_sent)) {
      // Restart comunication
      log.log(Level.FINEST, "Received Nack("+first+","+last+") for message not sent. Restarting communication.");
    ...
    if (first <= peer.last_msg_confirmed) {
      if (last <= peer.last_msg_confirmed) {
        log.log(Level.FINEST, "Received Nack for messages already confirmed. Discarding.");
        return;
      }
      first=peer.last_msg_confirmed+1;
      log.log(Level.FINEST, "Received Nack for message already confirmed. Changig first to "+first);
    }
    
    if (last > peer.last_msg_sent) {
      log.log(Level.FINEST, "Nack includes messages not sent to peer. Sending Update.");
      if (first <= peer.last_msg_sent) {
        log.log(Level.FINEST, "Nack partially includes messages sent to peer, resending.");
    ...
    while (peers_iter.hasNext()) {
      ...
      if ((peer != null) && (peer.rounds_msg_sent > param_MAX_SENT_ROUNDS)) {
        try {
          ...
          ex.printStackTrace();
          log.log(Level.WARNING, "Impossible to send ping.");
        } catch (CloneNotSupportedException ex) {
          ex.printStackTrace();
          log.log(Level.WARNING, "Impossible to send ping.");
        }
  ...
  private void handleIgnore(IgnoreEvent ev) {
    ...
    //if (debugFull)
    log.log(Level.FINEST, "Received Ignore from "+peer.addr.toString()+" with value "+peer.last_msg_delivered);
  }
  ...
  private void handleUpdate(UpdateEvent ev) {
    ...
    if ((ev.from=utils.popSeq(ev.getMessage(),peer.last_msg_delivered, false)) < 0) {
      log.log(Level.FINEST, "Received incorrect Update. Discarding.");
      return;
    }
  ...
  private void handleConfirm(ConfirmEvent ev) {
    Peer peer=(Peer)peers.get(ev.source);
    if (peer == null) {
    	log.log(Level.FINEST, "Received Confirm from unknown peer ("+ev.source+"). Discarding it.");
    	return;
    }
  ...
  private void receive(Peer peer, SendableEvent ev, long seqfrom, long seqto) {    
    ...
    if (debugFull)
    	log.log(Level.FINEST, "Received event "+ev+" from "+peer.addr+" with seq "+seqfrom+" -> "+seqto);
    ...
    if ((seqfrom <= peer.last_msg_delivered+1) && (seqto >= peer.last_msg_delivered+1)) {
      ...
      if (seqto <= peer.last_msg_delivered) {
        log.log(Level.FINEST, "Received old message from "+peer.addr.toString()+". Discarding.");
        return;
      }
   
      if (debugFull)
    	  log.log(Level.FINEST, "Storing undelivered from "+peer.addr+" with seq "+seqfrom);
  ...
  private void confirmed(Peer peer, long peer_confirmed, Channel channel) {
    ...
    if ((peer_confirmed >= peer.first_msg_sent) && (peer_confirmed <= peer.last_msg_sent)) {
      ...
      if (peer_confirmed > last_msg_sent) {
        log.log(Level.FINEST, "Received wrong peer confirmed number (expected between "+peer.first_msg_sent+" and "+last_msg_sent+", received "+peer_confirmed+" from "+peer.addr+". Sending Ignore.");
        ignore(peer,channel);
      }
    }
  }
  ...
  private void nack(Peer peer, long first, long last, Channel channel) {
    ...
    try {
      ...
      ex.printStackTrace();
      log.log(Level.WARNING, "Impossible to send Nack. Maybe next time.");
      return;
    }
    peer.nacked=new Nacked(first,last);
    
    // TODO erase
    log.log(Level.WARNING, "nacked: "+first+" - "+last+" ("+(last-first)+")");
  ...
  private void ignore(Peer peer, Channel channel) {
    try {
      ...
      ev.go();
      if (debugFull)
        log.log(Level.FINEST, "Sent Ignore with "+peer.last_msg_confirmed+" to "+peer.addr);
    } catch (AppiaEventException ex) {
      ex.printStackTrace();
      log.log(Level.WARNING, "Unable to send Ignore later it will be retransmited.");
      return;
    }
    peer.rounds_msg_sent=0;
  }
  ...
  private void update(Peer peer, long to, Channel channel) {
    try {
      ...
      ex.printStackTrace();
      log.log(Level.SEVERE, "Unable to send or store update.");
  ...
  private void confirm(Peer peer) {
    try {
        ...
        ex.printStackTrace();
        log.log(Level.WARNING, "Unable to send ConfirmEvent. Continuing.");
        return;
    }
    peer.last_confirm_sent=peer.last_msg_delivered;
    if (debugFull)
        log.log(Level.FINEST, "Sent Confirm "+peer.last_confirm_sent+" to "+peer.addr);
  }
  ...
  private void storeUnconfirmed(Peer peer, SendableEvent ev) {
    ...
    if (((size / 500) > 0) && ((size % 500) == 0))
        log.log(Level.WARNING, "Unconfirmed reached "+peer.unconfirmed_msgs.size());
  }
...
//            ev.getMessage().popInt();
    ...
    while (aux.hasPrevious()) {
      ...
      if (evaux instanceof UpdateEvent) { 
    	  ...
    	  if ((seq >= update.from) && (seq <= update.to)) {
    		  log.log(Level.FINEST, "Received undelivered message already stored. Discarding new copy.");
    		  return;
    	  }
    	  seqaux=update.to;
      }  else {
    ...
    while (aux.hasNext()) {
      ...
      if (evaux instanceof UpdateEvent) {
        ...
        if (update.to <= peer.last_msg_delivered) {
        	log.log(Level.FINEST, "Discarded unwanted event from "+peer.addr+" with seq "+update.from+" -> "+update.to);
        	aux.remove();
        ...
        if (seqaux <= peer.last_msg_delivered) {
        	log.log(Level.FINEST, "Discarded unwanted event from "+peer.addr+" with seq "+seqaux);
        	aux.remove();        	
          ...
          if (!(evaux instanceof PingEvent)) {
            try {
              ...
              ex.printStackTrace();
              log.log(Level.FINEST, "Discarding event "+evaux+". This may lead to incoherence.");
            }
          }
  ...
  private void sendFIFOUndelivered(SendableEvent ev, Object addr) {
    ...
    try {
      ...
      ex.printStackTrace();
      log.log(Level.WARNING, "Unable to send Undelivered notification. Continuing but problems may happen.");
    } catch (CloneNotSupportedException ex) {
      ex.printStackTrace();
      log.log(Level.WARNING, "Unable to send Undelivered notification. Continuing but problems may happen.");
    }
  ...
  private void sendTimer(Channel channel) {
    try {
      ...
      //ex.printStackTrace();
      log.log(Level.WARNING, "Unable to send timer. Correct operation of session is not guaranteed.");
    }
  }
  ...
  private void registerAddr(Object addr) {
    ...
    myHashesLen++;
    log.log(Level.FINEST, "Registered "+addr+" with hash "+myHashes[myHashesLen-1]);
  }
  ...
  */
  ...
  private void debugPeer(Peer peer, String s) {
      if (debugFull && Level.FINEST.equals(log.getLevel())) {
