...
public class NakFifoSession extends Session implements InitializableSession {
    private static Logger log = Logger.getLogger(NakFifoSession.class.getName());
    ...
    public void handle(Event event) {
        ...
        if (event instanceof NackEvent) {
            ...
            log.log(Level.SEVERE, "Unwanted event (\""+event.getClass().getName()+"\") received. Continued...");
    ...
    private void handleSendable(SendableEvent ev) {
        ...
        log.log(Level.WARNING, "Direction is wrong. Discarding event "+ev);
    }
    ...
    private void handleNack(NackEvent ev) {
        ...
        if ((first < peer.first_msg_sent) || (last > peer.last_msg_sent)) {
            ...
            log.log(Level.FINEST, "Received Nack for message not sent. Restarting communication.");
    ...
    private void receive(SendableEvent ev) {
        ...
        if ((flags & MessageUtils.IGNORE_FLAG) != 0) {
            log.log(Level.FINEST, "Received msg with ignore flag. Ignoring.");
            try { ev.go(); } catch (AppiaEventException ex) { ex.printStackTrace(); }
            return;
        }
        ...
        if ((seq=utils.popSeq(ev.getMessage(),peer.last_msg_delivered,false)) < 0) {
            log.log(Level.FINEST, "Problems reading sequence number discarding event "+ev+" from "+ev.dest.toString());
            return;
        }

        long peer_confirmed;
        if ((peer_confirmed=utils.popSeq(ev.getMessage(),peer.last_msg_confirmed,false)) < 0) {
            log.log(Level.FINEST, "Problems reading last message received by peer, discarding event "+ev+" from "+ev.dest.toString());
            return;
        }
        ...
        if ((peer_confirmed >= peer.first_msg_sent) && (peer_confirmed <= peer.last_msg_sent)) {
            ...
            if ((peer_confirmed > 0) && (peer_confirmed != peer.last_msg_confirmed)) {
                log.log(Level.FINEST, "Received wrong peer confirmed number (expected between "+peer.first_msg_sent+" and "+peer.last_msg_sent+", received "+peer_confirmed+". Sending Ignore.");
                ignore(peer,ev.getChannel());
            }
        }
    ...
    private void sendTimer(Channel channel) {
        try {
            ...
            //ex.printStackTrace();
            log.log(Level.SEVERE, "Unable to send timer. Corrcet operation of session is not guaranteed.");
        }
    }
